...
![创建应用#postpic/pica/hexo-leancloud-plugin-installation-tutor/1.png]()
然后弹出如下窗口：起名字，选择开发版，之后点击创建按钮  
![创建应用#postpic/pica/hexo-leancloud-plugin-installation-tutor/2.png]()
创建完之后，我们看到下面这样。然后点击右上角的设置  
![应用设置#postpic/pica/hexo-leancloud-plugin-installation-tutor/3.png]()
进入之后，我们点击存储，创建Class，弹出的对话框中名字要写`Counter`，必须写Counter是因为需要和NexT主题兼容。然后ACL权限选择无限制，不然容易统计次数不正常。最后点击创建Class按钮。  
![创建Class#postpic/pica/hexo-leancloud-plugin-installation-tutor/4.png]()
创建Class的时候容易出现这个问题，如下图，刷新几次，就好了。  
![问题#postpic/pica/hexo-leancloud-plugin-installation-tutor/5.png]()
现在获取应用ID和应用Key。设置，应用key。  
![ID和Key#postpic/pica/hexo-leancloud-plugin-installation-tutor/6.png]()
...
![Counter类未初始化#postpic/pica/hexo-leancloud-plugin-installation-tutor/7.png]()
然后它提示我们看F12的Console，显示信息如下：  
![F12Console#postpic/pica/hexo-leancloud-plugin-installation-tutor/8.png]()
...
![查看用户#postpic/pica/hexo-leancloud-plugin-installation-tutor/9.png]()
然后对Counter表设置权限，点击存储，Counter，其他，权限设置。如下：  
![设置权限#postpic/pica/hexo-leancloud-plugin-installation-tutor/10.png]()
然后弹出对话框，点击add_fields，指定用户，输入刚才用户，点击添加，  
![add_fields权限#postpic/pica/hexo-leancloud-plugin-installation-tutor/11.png]()
添加成功能够看到两处不同，用户ID已经上去了。  
![添加成功#postpic/pica/hexo-leancloud-plugin-installation-tutor/12.png]()
接下来，我们同样对create进行指定用户。  
![create权限#postpic/pica/hexo-leancloud-plugin-installation-tutor/13.png]()
然后，对delete指定用户，这个注意，不添加任何用户。然后关闭。  
![delete权限#postpic/pica/hexo-leancloud-plugin-installation-tutor/14.png]()
...
![查看记录#postpic/pica/hexo-leancloud-plugin-installation-tutor/22.png]()

# 安全中心保护
点击设置，安全中心，Web安全域名。这个保证只有对应域名传过来的才有效，也就是说，别人用你的应用ID和应用Key是无效的。  
![安全中心保护#postpic/pica/hexo-leancloud-plugin-installation-tutor/15.png]()

# 云引擎保护
云引擎保护访客数量不被随意篡改。  
点击云引擎，部署，在线编辑。  
![在线编辑#postpic/pica/hexo-leancloud-plugin-installation-tutor/16.png]()
然后点击创建函数。  
![创建函数#postpic/pica/hexo-leancloud-plugin-installation-tutor/17.png]()
...
```
然后点击保存。  
![Counter函数定义#postpic/pica/hexo-leancloud-plugin-installation-tutor/18.png]()
之后，我们能看到已经多了一个函数，然后点击上边的部署。  
![部署#postpic/pica/hexo-leancloud-plugin-installation-tutor/19.png]()
弹出对话框，点击部署  
![部署确认#postpic/pica/hexo-leancloud-plugin-installation-tutor/20.png]()
等待，直到部署完成，如下：  
![部署完成#postpic/pica/hexo-leancloud-plugin-installation-tutor/21.png]()
